...
cirMin = min(cir, [], [1 2], 'omitnan');
cirMax = max(cir, [], [1 2], 'omitnan');
cir = (cir - cirMin) ./ (cirMax - cirMin + eps);
...
function cir = generateCIR(rays,AP,STA,cfg,tx,ofdmInfo,snr,ofdmSymbolOffset)
    ...
    % Ensure input is N×1 column (SISO/SIMO)
    if size(tx,2) > 1
        tx = tx(:,1);  % Take only first antenna/stream
    end
    ...
    for i = 1:length(snr)
        try
            ...
            warning("Failed to simulate ray tracing channel: %s", ME.message);
            continue;
        end
        try
            rx = awgn(rxChan, snrAdj(i));
            chanEst = heRangingSynchronize(double(rx), cfg);
            if isempty(chanEst)
                continue
            end
            ...
            cir(:,:,i) = reshape(abs(cirRaw(1:ofdmSymbolOffset * ofdmInfo.CPLength,:,:)), ...
                                 ofdmSymbolOffset * ofdmInfo.CPLength, []);
        catch ME
            warning("Failed during CIR extraction: %s", ME.message);
        end
    end
end
