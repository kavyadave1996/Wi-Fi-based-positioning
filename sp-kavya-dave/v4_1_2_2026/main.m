...
%% ------------------------------------------------------------------
                ...
                disp("DEBUG: categories(labels.class) AFTER removing class 0:");
                disp(categories(labels.class));
                
                disp("DEBUG: countcats(labels.class) AFTER removing class 0:");
               ...
               % ==== DATASET AUGMENTATION: FEATURES + LABELS TOGETHER ====
                ...
                % trainIdx = find(~isVal);
                % valIdx   = find(isVal);
                % --- Stratified Split by STA (Ensured Safety Fix) ---
                % --- Stratified Split by STA (Ensured Dimension Safety) ---
                valFrac = 0.2;
                trainIdx = [];
                valIdx = [];
                
                % Remove categories with zero samples to prevent empty indexing
                labels.class = removecats(labels.class);
                uniqueClasses = categories(labels.class);
                
                for i = 1:numel(uniqueClasses)
                    thisRoomSampleIdx = find(labels.class == uniqueClasses(i));
                    
                    if isempty(thisRoomSampleIdx)
                        continue;
                    end
                    
                    stasInThisRoom = unique(sample2sta(thisRoomSampleIdx));
                    numStasInRoom = numel(stasInThisRoom);
                    
                    if numStasInRoom == 0
                        continue;
                    end
                    
                    permSTA = stasInThisRoom(randperm(numStasInRoom));
                    
                    % Select at least 1 STA for validation, but no more than available
                    nValSTA = max(1, round(valFrac * numStasInRoom));
                    nValSTA = min(nValSTA, numStasInRoom);
                    
                    roomValSTAs = permSTA(1:nValSTA);
                    roomTrainSTAs = permSTA(nValSTA+1:end);
                    
                    % Find sample indices for these STAs
                    thisRoomValIdx = thisRoomSampleIdx(ismember(sample2sta(thisRoomSampleIdx), roomValSTAs));
                    thisRoomTrainIdx = thisRoomSampleIdx(ismember(sample2sta(thisRoomSampleIdx), roomTrainSTAs));
                    
                    % CRITICAL FIX: Force column orientation (:) before vertical concatenation (;)
                    valIdx = [valIdx; thisRoomValIdx(:)];
                    trainIdx = [trainIdx; thisRoomTrainIdx(:)];
                end
                
                % Flatten final vectors
                trainIdx = trainIdx(:);
                valIdx = valIdx(:);
               ...
               % === CORRECTED SPLIT LOGIC ===
                if ~isempty(missingInVal)
                    fprintf("NOTE: Validation set is missing classes: %s. This is normal for small datasets.\n", strjoin(missingInVal, ", "));
                end
       ...
       % Check for empty fields in allAccuracy and allPositioningCDF
disp('Checking allAccuracy and allPositioningCDF structures:');
disp(fieldnames(allAccuracy));
disp(fieldnames(allPositioningCDF));

% Assuming 'allAccuracy' and 'allPositioningCDF' contain data
% Build a 2xA matrix of accuracies (rows = distributions, cols = antenna sizes)
...
for d = 1:numel(distribution)
    ...
    for i = 1:numel(antennaSizes)
        ...
        if isfield(allAccuracy, dn) && isfield(allAccuracy.(dn), num2str(antennaSizes(i)))
            accMat(d, i) = mean(allAccuracy.(dn).(num2str(antennaSizes(i))));
        else
            disp('No accuracy data found for this configuration.');
        end
    end
end
