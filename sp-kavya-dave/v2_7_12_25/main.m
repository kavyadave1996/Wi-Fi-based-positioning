...
for distIdx = 1:numel(distribution)
    ...
    for sepIdx = 1:numel(staSeparations)
        ...
        for matIdx = 1:numel(materials)
            ...
            for fusIdx = 1:numel(fusionStrategies)
               ...
               % ==== DATASET AUGMENTATION: FEATURES + LABELS TOGETHER ====
                ...
                for i = 1:numSamples
                    % Pick which original sample to copy/augment
                    sampleIdx = mod(i-1, numOriginal) + 1;
                
                    % --- Features: add noise ---
                    oneSample = features(:,:,:,sampleIdx);
                    features_aug(:,:,:,i) = oneSample + noiseLevel * randn(size(oneSample));
                
                    % --- Labels: copy (same class and position) ---
              ...
              %% ========= RESNET-18 BASED LOCALIZATION (CLASSIFICATION) =========
                ...
                validation.Y.classification = removecats(validation.Y.classification);
                ...
                % Targets as N×3
                trainY = training.Y.regression.';     % N×3
                valY   = validation.Y.regression.';   % N×3
                
                options.ValidationData = {validation.X, valY};
                
                fprintf('Training regression network...\n');
                netPos = trainnet(training.X, trainY, dlNetReg, lossFcn, options);
            ...
            distName = distribution{distIdx};
            allAccuracy.(distName)        = accuracyArray;
            allPositionError.(distName)   = meanDistErrArray;
            allPositioningCDF.(distName)  = positioningCDF;
        ...
        for d = 1:numel(distribution)
            dn = distribution{d};
            allAccuracy.(dn)      = zeros(1,numel(antennaSizes));
            allPositionError.(dn) = zeros(1,numel(antennaSizes));
        end
